...
const planRoot = resolve(repositoryRoot, "benchmarks", "temporal-classifier");
const defaultOutput = resolve(planRoot, "rule-mmaction2-comparison-plan-v1.json");

export const RULE_TEMPORAL_PLAN_FORMAT =
  "vcg-rule-temporal-classifier-comparison";
export const RULE_TEMPORAL_PLAN_VERSION = 1;
export const RULE_TEMPORAL_CAMPAIGN_ID = "core17-rule-mmaction2-v1";
export const RULE_TEMPORAL_LABELS = [
  "jump",
  "squat",
  "lean_left",
  "lean_right",
...
function requireBoundedOutput(argument) {
  ...
  if (
    ...
    throw new Error(
      "output must be a JSON file below benchmarks/temporal-classifier",
    );
  }
  return output;
}

export function buildRuleTemporalComparisonPlan() {
  return {
    format: RULE_TEMPORAL_PLAN_FORMAT,
    formatVersion: RULE_TEMPORAL_PLAN_VERSION,
    documentType: "plan",
    campaignId: RULE_TEMPORAL_CAMPAIGN_ID,
    generatedAt: new Date().toISOString(),
    sourceCommit: git(["rev-parse", "HEAD"]),
    workingTreeClean: git(["status", "--porcelain"]) === "",
    claimBoundary:
      "Architecture and evidence plan only. No MMAction2 environment, custom VCG dataset, training run, checkpoint, held-out participant, accuracy result, runtime, package size, target platform, or model selection exists.",
    upstream: {
      project: "open-mmlab/mmaction2",
      release: "v1.2.0",
      documentationRevision: "4d6c9347",
      releaseUrl:
      ...
      supportedKeypointFormats: ["coco", "nturgb+d", "openpose"],
      dependencyBoundary:
    ...
    labels: RULE_TEMPORAL_LABELS,
    dataset: {
      inputProfile: "Motion 0.4.0 body.core17 normalized 2D skeleton-only",
      ...
      attemptsPerPositiveLabelPerParticipant: 20,
      negativeMinutesPerParticipant: 15,
    ...
    training: {
      ...
      repeatedRunsPerTemporalCandidate: 3,
      rulesTrainingPolicy:
        "freeze calibration, thresholds, hysteresis, cooldown, and lifecycle adapter before held-out evaluation",
      temporalTrainingPolicy:
        "train custom 11-label heads only; validation may select epoch and thresholds; held-out test is single-use",
      classBalancePolicy:
        "report natural prevalence and a separately balanced training sampler; never rebalance held-out metrics",
      earlyStoppingSource: "validation split only",
      prohibitedLeakage: [
    ...
    evaluation: {
      ...
      requiredResourceMetrics: [
        "training wall time",
        "training peak RAM and VRAM",
        "checkpoint bytes",
        "runtime package bytes",
        "cold-load time",
        "per-window inference p50/p95/p99/worst",
        "full camera-to-action p50/p95/p99/worst",
        "runtime CPU/RAM/GPU/VRAM",
      ],
      requiredExplainabilityArtifacts: [
        "rule threshold and state transition trace",
        "temporal per-class score timeline",
        "temporal joint/time attribution with method and limitations",
        "bounded skeleton-only false-positive and false-negative exemplars",
      ],
      reportSlices: [
        "participant",
        "persona class",
      ...
      selectionPolicy:
        "no candidate selection from aggregate accuracy alone; every blocking persona and privileged false-event gate must pass, resource/package costs must fit both target tiers, and accessibility results remain separate",
      minimumHeldOutParticipantsPerBlockingClass: null,
      perLabelMetricGates: null,
      resourceGates: null,
    },
    requiredArtifacts: [
      "consent and minimization protocol hash",
      "dataset manifest and every skeleton-trace hash",
      "participant-disjoint split manifest hash",
      "independent label manifest and adjudication hash",
      "exact MMAction2 repository/tag/revision and dependency lock hashes",
      "custom dataset adapter, configs, label map, transforms, and seeds",
      "rule implementation and parameter hashes",
      "every trained checkpoint and training-log hash",
      "per-run and aggregate held-out metrics without omitted failures",
      "runtime/package/resource measurements on ordinary x86-64 Linux and Raspberry Pi",
      "license, dependency, model-weight, and dataset-rights review",
      "explicit no-selection or selected-candidate decision record",
    ],
  };
}
...
if (fileURLToPath(import.meta.url) === resolve(process.argv[1] ?? "")) {
  ...
  const plan = buildRuleTemporalComparisonPlan();
