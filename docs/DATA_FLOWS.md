...
| Developer workstation trust, session, and inert Push receipt | Future reserved-input confirmation and protected-state adapter → strict native registry → volatile signed challenge → closed operation capability → developer-only artifact store | Canonical public-key registry is designed for persistent host state and bound to external protected generation/digest; session epoch, challenge, capability, and replay set are volatile only; an authorized Push may atomically publish one canonical path-free receipt plus inert exact-hash blob in a disjoint preprovisioned store | Future encrypted paired-LAN receiver and developer installer/runtime; no current network recipient, parser, catalog, or process | Local pair/revoke and developer-mode enable/exit; explicit safe cleanup of incomplete staging; no browser or game authority | Native trust/session admission and whole-artifact receipt implemented; persistence/protected keys/state, listener/encryption, active-transfer cancellation/retry, UI, archive/install/operations/retention/audit are not |
