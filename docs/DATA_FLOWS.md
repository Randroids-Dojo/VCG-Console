...
| Developer workstation trust and session | Future reserved-input confirmation and protected-state adapter → strict native registry → volatile signed challenge → closed operation capability | Canonical public-key registry is designed for persistent host state and bound to external protected generation/digest; session epoch, challenge, capability, and replay set are volatile only | Future encrypted paired-LAN receiver and developer-only deployment service; no current network recipient | Local pair/revoke and developer-mode enable/exit; no browser or game authority | Native trust/session admission implemented; persistence adapter, protected keys/state, listener, encrypted transport, UI, artifacts, operations, and audit are not |
