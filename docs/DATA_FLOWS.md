...
| Portable skeleton and actions | Pose result → MediaPipe adapter → Motion `0.2.0` frame → action engine/render/game | In-memory current frame plus a bounded 600-frame `TraceBuffer`; cleared on input-mode changes. No automatic persistent storage. | Shell, local obstacle game, and qualified cooperative game profiles | Camera stop; replay switch; explicit trace export | Implemented |
| Rich/world skeleton extensions | MediaPipe adapter capability profiles | Same bounded in-memory treatment as the portable frame | Only a bridge client that negotiated the corresponding profile | Per-game manifest/allowlist is planned; bridge profile negotiation exists | Partially implemented |
