...
Raw video should be processed locally, not recorded, and not transmitted by default. Diagnostics should export skeletal traces only after explicit admin consent, with a clear deletion path. Device-only portraits, calibration fields, and body-profile-matching measurements live in an automatically unlocked encrypted vault protected by a console-bound key. This protects a removed or cloned storage card without adding a family login step, but does not protect plaintext while an authorized running console is using it.

The vault runtime is deny-by-default. A narrow profile broker is the only process allowed to unseal the console-bound key or decrypt vault records. The launcher may request display names, a separately approved portrait rendering handle, and coarse calibration readiness; it never receives body-matching measurements. Native games receive only an opaque active-player ID and explicitly granted gameplay calibration outputs through versioned IPC. Web games receive neither vault fields nor portrait handles and see only the already-negotiated Motion API. Diagnostics receive aggregate health and redacted field-presence information, never portraits, calibration values, feature vectors, or matching inputs. Each request is authorized by caller identity, operation, profile, and session against an explicit ACL; absent grants fail closed. No game-visible IPC method can enumerate or fetch portraits, calibration data, or body-profile-matching measurements.

The key and plaintext must not enter backups, recovery images, diagnostics, logs, swap, crash dumps, or game storage. There is no recovery copy: key loss destroys the vault and requires profile recreation. Console-managed saves are separately isolated but are also intentionally device-local and unrecoverable after storage loss. The selected baseline requires a physical optical shutter while camera power and activity remain software-managed. The UI must not claim to know whether the shutter is open or closed unless the exact hardware exposes a trustworthy sensor.
