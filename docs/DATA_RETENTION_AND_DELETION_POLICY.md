...
| Opaque profile registry record | Strict host registry now carries opaque IDs only | Persistent host state by deployment; no mutation service | Future exact create/delete transaction | Removing an ID alone is not complete deletion | Strict bounded parser and launch integration implemented; mutation/protected-state binding absent |
