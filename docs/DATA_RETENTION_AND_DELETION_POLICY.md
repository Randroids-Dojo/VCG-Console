...
| Native diagnostic foundation | Unwired Rust store requires explicit event, byte, and boot-epoch bounds and retains only closed path-free records plus empty producer-time watermarks | Oldest complete records are removed after event/watermark publication; incomplete incoming records are removed on reopen | Low-level complete clear uses a durable marker and resets events, watermarks, sequence, eviction, and time; admin confirmation is not implemented | Local only; snapshot is not serializable and grants no upload/share authority | Bounded persistence, time floors, logical crash recovery, and deletion mechanics implemented; runtime ownership, OS source authentication, trusted boot provenance, final duration/size policy, admin/export workflow, and target evidence unresolved |
| Accessibility preferences | At most 1,024 bytes in `vcg.accessibility.v1` browser storage; unwired native store retains one newest exact-v1 generation after cleanup | Invalid latest state defaults in full; incomplete native incoming state is removed and superseded generations are cleaned on reopen/save | Reset accessibility settings; native reset marker makes complete reset restart-recoverable | Device-wide prototype only; not profile deletion scope | Browser prototype and native persistence boundary implemented; production service/launcher wiring, browser transition, final scope, and target fault evidence absent |
