...
| Native diagnostic foundation | Unwired Rust store requires explicit event, byte, and boot-epoch bounds and retains only closed path-free records | Oldest complete records are removed after crash-safe publication; incomplete incoming records are removed on reopen | No clear or export authority exists | Local only; snapshot is not serializable and grants no upload/share authority | Bounded persistence and logical crash recovery implemented; runtime ownership, OS source authentication, trusted boot provenance, final duration/size policy, admin clear/export, and target evidence unresolved |
