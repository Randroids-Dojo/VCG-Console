...
`schemas/game-manifest.schema.json` and `schemas/installed-game-manifest.schema.json` are both generated by `pnpm prepare:schemas` and gated by `pnpm validate:schemas`. The installed-root export is additionally compared byte-for-byte by a `packages/game-manifest` test, so `pnpm -r test` rejects a stale export as well.

The exported installed-root schema carries what the parser enforces rather than a weaker projection of it: artifact sources are constrained to HTTPS by pattern, because a Zod refinement alone does not survive `z.toJSONSchema()` and would leave the published schema accepting an `http://` source the parser refuses. A library binding's `systemId` and `coreId` cap at 64 characters, matching the installed-library grammar they must resolve against, rather than the 80 the document's other identifiers allow.
