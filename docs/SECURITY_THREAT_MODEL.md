...
The Rust library now verifies every signed-generation artifact before publishing a digest-bound durable intent and again after the generation move, rejects equal/lower generations while activation history is intact, atomically publishes append-only markers, and resumes interruption at every durable boundary while keeping managed content and saves outside the tree. Normal launcher startup recovers and re-verifies the active store before opening the API/browser; dry-run detects pending recovery without mutating it. The remaining trust chain is intentionally incomplete: public-key provisioning is configurable; writable activation history is not tamper-resistant; channel identity and authenticated anti-rollback recovery are undefined; candidate ingestion, capacity reservation, health gating, cleanup, and uninstall do not exist; non-Unix directory durability and target power-loss behavior are unqualified; and verification-to-execution immutability is not proven. Production design still needs offline-root/online-role separation, revocation and threshold policy, protected monotonic per-channel state, read-only versioned package mounts or descriptor-based execution, crash/power-loss campaigns, bad-release rollback, and a separately visible developer namespace.
