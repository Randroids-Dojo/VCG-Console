...
The Rust library verifies every signed-generation artifact before intent and after move, extracts bounded health only from the re-hashed manifest, runs candidates with ephemeral runtime/data roots, requires signed process-survival or explicit-ready behavior, reaps every child, and rechecks the exact catalog digest before append-only activation. Normal launcher startup recovers/re-verifies active state; dry-run stays read-only. A path-free cleanup planner performs no deletion. The remaining trust chain is intentionally incomplete: public-key provisioning and activation history are writable; channel/authenticated anti-rollback policy is undefined; an ordinary child can self-assert ready until exact producers are qualified; candidate intake, capacity, post-activation health/rollback, cleanup execution, and uninstall do not exist; non-Unix durability and target behavior are unqualified; and verification-to-execution immutability is unproven. Production still needs offline-root roles, revocation, protected monotonic state, read-only mounts/descriptor execution, hostile health-producer tests, crash/power-loss campaigns, bad-release rollback, and a separate developer namespace.
