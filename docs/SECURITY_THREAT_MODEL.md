...
The Rust library now binds a resumable exact-offset transfer to signed archive evidence, derives progress from synchronized bytes, rejects gaps/conflicting replay, rechecks remaining-byte headroom, and publishes only a fully hashed no-replace ready archive whose immutable release binding is retained. The generation-store handoff keeps the receiver lock, independently verifies the descriptor with the store key, requires the same binding, and re-hashes the ready archive. Intake binds exact archive/expanded/catalog evidence, rejects unsafe TAR paths/types/collisions/modes, extracts into private no-replace work, fully verifies artifacts, and publishes only inert staging while retaining the ready receipt. Candidate health, move-time reverification, append-only activation, launcher recovery, and read-only cleanup planning remain fail-closed. Cooperating store mutations now use one nonblocking inert lock, and live cleanup planning freezes launch admission before it takes that store lock and derives generation protection; browser/package callers cannot supply the protected set. The trust chain is still incomplete: network discovery/TLS/range behavior does not exist; public-key provisioning and activation history are writable; release/catalog role separation and authenticated anti-rollback are undefined; capacity is checked rather than reserved; compressed extraction does not exist; an ordinary child can self-assert ready until producers are qualified; post-activation rollback, cleanup execution, and uninstall do not exist; hostile same-account writers, non-Unix durability, target lock behavior, and verification-to-execution immutability are unproven. Production still needs offline-root roles, revocation, protected monotonic state, real storage reservation, read-only mounts or descriptor execution, hostile transfer/intake/health tests, crash/power-loss campaigns, and a separate developer namespace.
