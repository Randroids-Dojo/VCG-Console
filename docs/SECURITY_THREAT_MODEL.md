...
8. **Rust host to child game.** [`process.rs`](../native/vcg-host/src/process.rs), [`retroarch.rs`](../native/vcg-host/src/retroarch.rs), [`native_package.rs`](../native/vcg-host/src/native_package.rs), and the native launch coordinator start direct children without a shell, verify catalog-selected artifacts, isolate supplied per-profile paths, observe exit, and terminate/reap owned direct children. Native packages cannot select arguments, environment names, working directories, or writable roots, but the process still inherits the service environment and can reach ambient filesystem/network/devices without a target sandbox. Host-selected installed games may require bounded heartbeat/process-exit recovery inside the same idempotent request, including cancellation during an attempt or restart backoff. The connected API path deliberately has no resource-fault input yet. An unwired Linux candidate retains hierarchical cgroup-v2 `memory.events`, baselines `oom_kill` per attempt, and checks terminal evidence before generic exit classification; it grants no authority until the service proves exact scope creation, attachment, containment, controller policy, and non-reuse under Q-247/Q-248. Immutable hash-to-spawn handoff, descendant process groups, sandboxing, compositor association, window readiness, production probe ownership, GPU evidence, and hostile-child cancellation are not proven.
...
12. **Ordinary runtime to power and boot maintenance.** The Rust power coordinator closes fresh native launch admission before calling any service, serializes the closure against direct and watchdog activation, binds host-selected closed adapter calls and handoff to one bounded epoch/operation, retains a non-droppable launch closure through idle/fault/handoff, and treats timeout, adapter failure, unsafe update state, and electrical interruption as failure rather than success. Only exact wake readiness consumes the closure. The traits are privileged adapter boundaries, not authentication by themselves; production must keep concrete service/input/platform clients and coordinator wiring unavailable to browser, game, Motion, controller, LAN, and ordinary runtime authority. Service and recovery authority remains a separate cold-boot physical-control path: an initial qualified hold/release authorizes only service mode, while destructive recovery requires a fresh physical press/release and one-shot consumption.
...
The Linux cgroup-v2 memory-health candidate opens one exact hierarchical
`memory.events` control without following the final scope/control symlink and
retains that handle across path replacement. It requires one bounded canonical
`oom_kill` counter, snapshots it before each attempt, rejects reversal, and
lets only an increase override generic process exit as `out-of-memory`.
Malformed, unavailable, or unreset evidence cannot manufacture OOM authority.
This does not establish that the correct processes were inside the scope:
without Q-247/Q-248 service binding, a game may escape, an unrelated process
may enter, an ancestor may supply the event, or a stale scope may be reused.
