...
System-image authority now begins with a separate domain-separated Ed25519 manifest. Signature verification precedes closed JSON parsing; canonical bounded metadata binds a safe release ID, exact privileged target, generation, raw file length, and SHA-256. The complete bounded regular file is opened once, length-checked before/after streaming, hashed, retained, and rewound for a future privileged writer. Changed/truncated images, wrong target/key/signature/schema/format, unknown fields, unsafe identifiers, and oversized inputs fail. Source verification creates no slot/journal evidence; only a complete matching read-back can create sealed evidence for a host-selected inactive slot. Retaining the handle prevents path replacement from redirecting the copy but does not stop concurrent in-place writes; writer-side reverify and synchronized inactive-partition read-back remain mandatory. This does not define trusted key roles, write a partition, choose which slot is inactive, or verify firmware.
