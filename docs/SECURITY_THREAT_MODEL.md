...
The Rust library now verifies a small domain-separated release descriptor before archive use, binds exact archive/expanded/catalog evidence, admits nonzero-reserve capacity, rejects unsafe TAR paths/types/collisions/modes, extracts into private no-replace work, fully verifies artifacts, and publishes only inert staging. It then health-gates exact signed policy before intent, re-verifies after the move, and activates append-only state. Launcher recovery/reverification and read-only cleanup planning remain fail-closed. The trust chain is still incomplete: public-key provisioning and activation history are writable; release/catalog role separation and authenticated anti-rollback are undefined; capacity is checked rather than reserved; network transfer and compressed extraction do not exist; an ordinary child can self-assert ready until producers are qualified; post-activation rollback, cleanup execution, and uninstall do not exist; destination concurrency, non-Unix durability, target behavior, and verification-to-execution immutability are unproven. Production still needs offline-root roles, revocation, protected monotonic state, serialized/reserved storage, read-only mounts or descriptor execution, hostile intake/health tests, crash/power-loss campaigns, and a separate developer namespace.
