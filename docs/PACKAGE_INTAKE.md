...
This document defines the implemented host-owned admission and extraction boundary between a completed package archive and the [signed generation store](PACKAGE_GENERATION_STORE.md). It does not implement network download, resume, mirrors, update scheduling, cleanup, or activation.
...
Network transport, resume/range semantics, TLS/mirror policy, real disk reservation, low-space UI and cleanup, `tar-zstd` decompression, hostile concurrent destination mutation, target-Linux permission/mount qualification, and sudden-power campaigns remain open. See [the owner intake questions](OWNER_QUESTIONS_PACKAGE_INTAKE_2026-07-23.md).
