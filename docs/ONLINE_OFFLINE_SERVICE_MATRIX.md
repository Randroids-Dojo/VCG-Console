...
| OS, host, model, catalog, and package updates | `maintenance-only` | Continue running the last verified installed versions indefinitely, subject to explicit security/revocation policy. | Check/download into staging, verify signatures/hashes, install inactive slot/package, health-check, promote, and roll back. | Dedicated update trust roots and host service; games cannot update themselves into family mode. | Signed package lifecycle, signature-first retained-handle system-image verification, sealed matching slot-read-back evidence, A/B state, and logical partition/capacity planning exist; network acquisition, key hierarchy, actual block writer/provenance, bootloader, mounts, and target qualification remain open. | Offline means “not checked,” not broken. Interrupted or invalid updates never replace the current verified version or consume writable recovery headroom as ordinary capacity. |
